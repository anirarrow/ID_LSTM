...
class LSTM_CriticNetwork(object):
    ...
    def __init__(self, sess, dim, optimizer, learning_rate, tau, grained, max_lenth, dropout, wordvector):
        ...
        #print(self.num_other_variables)
        #exit(0)
        #Create a variable with the name wordvector and initialize it with the vocab wordvectors (300 dim)
        ...
        #Get parameters of the entire model: with all layer shapes
        self.network_params = tf.trainable_variables()[self.num_other_variables:]
        print("%%%")
        print(self.num_other_variables)
        #exit(0)
        #Create a variable with the name target wordvector and initialize it with the vocab wordvectors (300 dim)
        ...
        #critic network (delayed updating): Get the input vector for a sentence, output of critic for a sentence, and length of input
        self.target_inputs, self.target_lenth, self.target_out = self.create_critic_network("Target")
        print(len(tf.trainable_variables()))
        print("%%%")
        #trainable_variables contain network layer weight matrices for two networks: some of them belong to network_params, the latter belong to target_network_params
        ...
        #delayed updating critic network ops: Update the target network params
        #update_target_network is a list of tensors that contain network weights again
        ...
        #assign_target_network is a list of tensors that contain network weights again
        self.assign_target_network_params = \
                [self.target_network_params[i].assign(\
                self.network_params[i]) for i in range(len(self.target_network_params))]
        #update_active_network is a list of tensors that contain network weights again
    ...
    def create_critic_network(self, Scope):
        #Critic network takes one sentence as input of length "lenth"
        ...
        #Lower network
        #Get vector representation of the sentence as input
        if Scope[-1] == 'e':
        ...
        with tf.variable_scope("Lower", reuse=True):
            #Dynamic RNN takes the input vector and the LSTM cell as input: Here, output has dimension (1,300,300)
            out, _ = tf.nn.dynamic_rnn(cell, vec, lenth, dtype=tf.float32, scope=Scope)


        #Get the output of the dynamic RNN: Here output has dimension (1,300) or (1,dim)
        out = tf.gather(out[0], lenth-1)


        out = tflearn.dropout(out, self.keep_prob)
        #Send the output to a fully connected layer: output has dimension (1,grained)
    ...
    def create_LSTM_cell(self,Scope):
        ...
        with tf.variable_scope(Scope, reuse=False):
            #Output and next state are functions of input and previous state
    ...
    def wordvector_find(self, inputs):
        #print(inputs)
        #print(self.WVvec)
        return self.sess.run(self.WVvec, feed_dict={self.WVinput :inputs})
